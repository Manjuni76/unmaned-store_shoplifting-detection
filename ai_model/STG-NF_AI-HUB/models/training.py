...
class Trainer:
    ...
    def train(self, log_writer=None, clip=100):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, data_arr in enumerate(pbar):
                try:
                    if itern == 0:
                        print("[DBG] start batch 0: moving tensors to device …")
                    data = [data.to(self.args.device, non_blocking=True) for data in data_arr]
                    
                    # 첫 배치만 출력
                    if not sample_shown:
                        print(f"[TRAIN-DATA] 데이터 형태: {data[0].shape}")
                        sample_shown = True
                    if itern == 0:
                        print("[DBG] after to(device): computing score …")
                    ...
                    if self.args.model_confidence:
                        ...
                        samp = data[0][:, :2]
                    if itern == 0:
                        print("[DBG] before forward: samp=", samp.shape, "score=", score.shape)
                    z, nll = self.model(samp.float(), label=label, score=score)
                    if itern == 0:
                        print("[DBG] after forward: nll is None?", nll is None)
                    if nll is None:
                        continue
                    if self.args.model_confidence:
                        nll = nll * score
                    ...
                    if itern == 0:
                        print("[DBG] after loss compute: ", float(losses.item()))
                    losses.backward()
                    if itern == 0:
                        print("[DBG] after backward")
    ...
    def test(self):
        ...
        for itern, data_arr in enumerate(pbar):
            ...
            # 첫 배치만 출력
            if not sample_shown:
                print(f"[TEST-DATA] 데이터 형태: {data[0].shape}, 점수 형태: {score.shape}")
                sample_shown = True
