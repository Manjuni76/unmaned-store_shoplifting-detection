...
class FlowStep(nn.Module):
    def __init__(
        ...
        self.subset_idx = subset_idx  # ★ 보관

        # --- 그래프 A 생성 ---
        if A is None:
            g = Graph(strategy=strategy, max_hop=max_hops)
            A = torch.from_numpy(g.A).float().to(device)  # (K, 18, 18)
        else:
            A = A.to(device).float()

        # 관절 부위별 슬라이싱
        if self.subset_idx is not None:
            print(f"[FLOW-STEP] 그래프 A 슬라이싱 전: {A.shape}")
            idx = torch.as_tensor(self.subset_idx, dtype=torch.long, device=device)
            A = A[:, idx][:, :, idx].contiguous()
            print(f"[FLOW-STEP] 그래프 A 슬라이싱 후: {A.shape}, subset_idx: {self.subset_idx}")
...
class FlowNet(nn.Module):
    def __init__(
        ...
        C, T, V = pose_shape
        print(f"[FLOW-NET] 초기화: pose_shape=({C}, {T}, {V}), subset_idx={self.subset_idx}")
        for i in range(L):
            if i > 1:
                C, T, V = C * 2, T // 2, V
...
class STG_NF(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        # kwargs에서 필요한 값 꺼내기 (기본값도 지정)
        LU_decomposed = kwargs.get('LU_decomposed', True)
        edge_importance = kwargs.get('edge_importance', False)
        temporal_kernel_size = kwargs.get('temporal_kernel_size', None)
        strategy = kwargs.get('strategy', 'uniform')
        max_hops = kwargs.get('max_hops', 8)
        device = kwargs.get('device', 'cuda:0')
        learn_top = kwargs.get('learn_top', False)
        
        # subset_idx가 있으면 pose_shape을 업데이트
        if subset_idx is not None:
            C, T, V = pose_shape
            print(f"[MODEL] 원래 pose_shape: ({C}, {T}, {V})")
            # V를 선택된 관절 수로 업데이트
            V = len(subset_idx)
            pose_shape = (C, T, V)
            print(f"[MODEL] 수정된 pose_shape: ({C}, {T}, {V})")
