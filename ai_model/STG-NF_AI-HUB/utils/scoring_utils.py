...
def score_dataset(score, metadata, args=None, save_results=True, directory="results/csv_files/"):
    ...
    # 모델 출력은 기본적으로 "정상(normality) 점수"(-NLL)가 들어옵니다.
    # 평가에서는 "이상(anomaly) 점수"가 양의 상관을 갖도록 변환합니다.
    scores_arr = smooth_scores(scores_arr)
    score_sem = getattr(args, 'score_semantics', 'normality') if args is not None else 'normality'
    if score_sem == 'normality':
        # normality -> anomaly 변환
        scores_arr = [(-s) for s in scores_arr]
...
def get_dataset_scores(scores, metadata, args=None, save_results=True, directory="results/csv_files/"):  
    ...
    elif args.dataset == 'Shoplifting':
        per_frame_scores_root = 'data/dataset_hdf5_ground_truth/test_gt'
        clip_list = sorted(fn for fn in os.listdir(per_frame_scores_root) if fn.endswith('_gt.npy'))
    else:
...
def get_clip_score(scores, clip, metadata_np, metadata, per_frame_scores_root, args):
    if args.dataset == 'UBnormal':
        ...
        # Parse scene_id and clip_id from Shoplifting GT filename
        # Examples:
        #   abnormal_shoplifting_001_C_3_12_16_..._gt.npy
        #   normal_shopping_001_C_1_1_10_..._gt.npy
        parts = clip.replace('_gt.npy', '').split('_')
        try:
            clip_id = int(parts[2])
        except Exception:
            clip_id = -1
    ...
    # GT의 양성 클래스(1)의 의미를 표준화: 평가에서는 항상 1=비정상으로 사용
    gt_pos = getattr(args, 'gt_positive', 'abnormal') if args is not None else 'abnormal'
    if gt_pos == 'normal':
        clip_gt = np.ones_like(clip_gt) - clip_gt
    ...
    for person_id in clip_fig_idxs:
        ...
        if person_metadata_inds.size == 0:
            continue
        pid_scores = scores[person_metadata_inds]
        pid_frame_inds = np.array([metadata[i][3] for i in person_metadata_inds]).astype(int)
        target_inds = pid_frame_inds + int(args.seg_len / 2)
        # 유효 범위 필터링 및 클리핑
        valid_mask = (target_inds >= 0) & (target_inds < clip_gt.shape[0])
        if not np.any(valid_mask):
            continue
        target_inds = target_inds[valid_mask]
        pid_scores = pid_scores[valid_mask]
        clip_person_scores_dict[person_id][target_inds] = pid_scores
