...
import pickle
import time
# args.py (argparse 정의 부분 근처)
import argparse



# 이미 있으면 default='arms' 로만 바꿔도 됨
...
def init_args():
    ...
    # ▼ 추가(옵션): 'none' → None
    if getattr(args, 'joint_subset', None) == 'none':
        args.joint_subset = None
    return init_sub_args(args)


def init_sub_args(args):
# 데이터 이름에 따라서 경로 지정 방법
 
    dataset = "Shoplifting" if args.dataset == "Shoplifting" else ("UBnormal" if args.dataset == "UBnormal" else ("PoseLift" if args.dataset == "PoseLift" else "ShanghaiTech"))
    if args.vid_path_train and args.vid_path_test and args.pose_path_train and args.pose_path_test:
        ...
        args.pose_path = {'train': os.path.join(args.data_dir, 'dataset_hdf5', 'train/'),
                          'test':  os.path.join(args.data_dir, 'dataset_hdf5', 'test/')}
    args.pose_path["train_abnormal"] = args.pose_path_train_abnormal
    args.ckpt_dir = None #모델 체크포인트 저장 경로 아직 지정X 
    model_args = args_rm_prefix(args, 'model_') # 모델 관련 인자들만 따로 분리함
    return args, model_args #그래서 초기화된 args와 모델 관련 인자들을 반환함


def init_parser(default_data_dir='data/', default_exp_dir='data/exp_dir'): #안에 기본 데이터 경로를 넣음
    parser = argparse.ArgumentParser(prog="STG-NF") #실행시 옵션들을 정의하는 함ㄴ수
    # General Args
    ...
    parser.add_argument('--pose_path_test', type=str, default='data/dataset_hdf5_ground_truth/test_gt', help='Path to test pose')
    parser.add_argument('--dataset', type=str, default='Shoplifting', 
                        choices=['ShanghaiTech', 'ShanghaiTech-HR', 'UBnormal', 'PoseLift', 'Shoplifting'], help='Dataset for Eval')
    parser.add_argument('--gt_positive', type=str, default='abnormal', choices=['abnormal', 'normal'],
                        help="GT에서 1이 어떤 클래스를 의미하는지 지정합니다. 기본='abnormal' (비정상=1)")
    parser.add_argument('--score_semantics', type=str, default='normality', choices=['normality','anomaly'],
                        help="모델 출력 점수의 의미를 지정합니다. normality(값↑=정상↑), anomaly(값↑=이상↑)")
    ...
    parser.add_argument('--num_workers', type=int, default=0, metavar='W', help='number of dataloader workers (0=current thread) (default: 32)')
    ...
    '--joint_subset',
    type=str,
    default='none',                 # ★ 기본 none
    choices=['arms', 'no_face', 'none'],
    help="관절 서브셋. arms=팔6관절, no_face=얼굴 제외, none=전신"
)
    ...
    parser.add_argument('--epochs', '-model_e', type=int, default=25, metavar='E', help = 'Number of epochs per cycle')
...
def create_exp_dirs(experiment_dir, dirmap='', joint_subset=None, seed=None):
    time_str = time.strftime("%b%d_%H%M")
    
    # 시드와 관절 부위 정보를 디렉토리 이름에 포함
    info_str = ""
    if joint_subset and joint_subset != 'none':
        info_str += f"_{joint_subset}"
    if seed is not None:
        info_str += f"_seed{seed}"
        
    experiment_dir = os.path.join(experiment_dir, dirmap, time_str + info_str)
    dirs = [experiment_dir]

    try:
        for dir_ in dirs:
            os.makedirs(dir_, exist_ok=True)
        print(f"실험 디렉토리 생성됨: {experiment_dir}")
