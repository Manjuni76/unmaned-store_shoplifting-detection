...
def normalize_pose(pose_data, **kwargs):
    ...
    :return:
    """
    vid_res = kwargs.get('vid_res', [1920, 1080])
    symm_range = kwargs.get('symm_range', False)
    # sub_mean = kwargs.get('sub_mean', True)
    ...
    pose_data_zero_mean = pose_data_centered
    # 평균/표준편차를 (T,V) 축으로 계산하고, 분모에 epsilon을 더해 0으로 나누는 것을 방지
    mean_tv = pose_data_centered[..., :2].mean(axis=(1, 2), keepdims=True)
    std_tv = pose_data_centered[..., :2].std(axis=(1, 2), keepdims=True)
    eps = 1e-6
    std_safe = np.maximum(std_tv, eps)
    pose_data_zero_mean[..., :2] = (pose_data_centered[..., :2] - mean_tv) / std_safe
    # 남은 NaN/Inf을 안전값으로 대체
    pose_data_zero_mean = np.nan_to_num(pose_data_zero_mean, nan=0.0, posinf=0.0, neginf=0.0)
